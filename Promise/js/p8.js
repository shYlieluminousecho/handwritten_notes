...
(function () {
    ...
    // 静态方法
    // 返回一个新的 Promise对象, 接收一个参数列表, 当所有的 Promise 实例都成功, 返回成功状态结果为每一个 Promise实例的成功的返回值
    Promise.all = function (taskArr) {
        return new Promise(function (resolve, reject) {
            // 保存每一个成功的 Promise 结果
            var result = []
            // 统计成功的个数
            ...
            // 封装函数 计算成功回调的结果
            function handleFulfilledResult(res, index) {
                count++
                result[index] = res
            ...
            taskArr.forEach(function (item, index) {
                ...
                if (item instanceof Promise) {
                    item.then(function (res) {
                        handleFulfilledResult(res, index)
                    }).catch(err => {
                        return err
                    })
                } else { // 非Promise对象
                    handleFulfilledResult(item, index)
                }
